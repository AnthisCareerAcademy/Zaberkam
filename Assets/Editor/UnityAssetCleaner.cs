...
{
    private Vector2 scrollPositionTop;
    private Vector2 scrollPositionBottom;
    private Dictionary<string, bool> assetsToDelete = new();
    private Dictionary<string, bool> scenesToInclude = new();
    ...
    private void RefreshSceneList()
    {
        ...
        // Add scenes from build settings
    ...
    private void OnGUI()
    {
        ...
        if (sceneFoldout)
        {
            ...
            {
                EditorGUILayout.BeginHorizontal();
                bool isIncluded = EditorGUILayout.Toggle(scene.Value);
    ...
    private void DrawAssetList()
    {
        if (assetsToDelete.Count > 0)
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            scrollPositionBottom = EditorGUILayout.BeginScrollView(scrollPositionBottom);

            foreach (var assetEntry in assetsToDelete.ToList().OrderBy(i => i.Key))
            {
                EditorGUILayout.BeginHorizontal();

                bool isMarked = EditorGUILayout.Toggle(assetEntry.Value);
                ...
                if (GUILayout.Button("X", redXStyle))
                {
            ...
            if (GUILayout.Button("Delete Marked Assets"))
            {
                DeleteMarkedAssets();
            }
